...
import { createScene, createTrajectory } from './system';
...
function downloadFile(name: string, contents: any, mime: string) {
...
class Viewer {
  ...
  gui: GUI;
  animator: Animator;
  bodyFolders: { [key: string]: GUI; };
  controlTargetPos: THREE.Vector3;
  selector: Selector;
  defaultTarget: THREE.Object3D<THREE.Event>;
  target: any;
  needsRender: boolean;



  constructor(domElement: HTMLElement, system: any) {
    ...
    for (let c of this.scene.children) {
      ...
      function defaults(...args: Controller[]) {
  ...
  setHover(object: any, hovering: boolean) {
    this.setDirty();
    if (!object.selected) {
      object.traverse(function (child: any) {
        if (child instanceof THREE.Mesh) {
          if (hovering && !child.baseMaterial) {
  ...
  setSelected(object: any, selected: boolean) {
    object.selected = selected;
    this.target = selected ? object : this.defaultTarget;
    object.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        if (selected && !child.baseMaterial) {
          child.baseMaterial = child.material;
        }
